...
void COPLL::UpdateVolExp(UINT8 ch)
{
	...
	attr->baseTL[1] = evol;
	evol = (evol >= 63) ? 15 : ((evol >> 2) & 0xf);
	SetReg(0x30 + ch, (GetReg(0x30 + ch, 0) & 0xf0) | evol);
}
...
void COPLL::UpdateTL(UINT8 ch, UINT8 op, UINT8 lev)
{
	...
	if (op == 0) {
		SetReg(0x2, (GetReg(2, 0) & 0xc0) | lev);
	} else if (op == 1) {
		SetReg(0x30 + ch, (GetReg(0x30 + ch, 0) & 0xf0) | (lev >> 2));
	}
}
...
void COPLL::RhythmOn(UINT8 num, UINT8 vel, SINT8 pan, FMVOICE* rv, FNUM* fnum)
 {
	...
	if (num < rhythmcap) {
		RhythmOff(num);
		UINT8 evol = CalcEffectiveLevel(rhythmvol, 127 - vel);
		evol = ((evol > 63) ? 63 : (evol & 63)) >> 2;
