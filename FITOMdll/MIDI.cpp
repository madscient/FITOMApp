...
//-----
CMidiInst::CMidiInst(CMidiIn* port, CFITOM* parent)
	: currentstatus(0)
{
	Parent = parent;
	Port = port;
...
void CInstCh::DataEntry(UINT8 data)
{
	if (!Device) { return; }
	switch(EntryMode)
	{
...
void CInstCh::DataIncrement(SINT16 data)
{
	if (!Device) { return; }
	switch (EntryMode)
	{
	case NRPN_ENTRY:
		switch (NRPNReg) {
		case NRPN_PMRATE:
			SetPMRate(GetPMRate() + data);
			break;
		case NRPN_AMRATE:
			SetAMRate(GetAMRate() + data);
			break;
		case NRPN_PHYCH:
			SetPhyCh(GetPhyCh() + data);
			break;
		case NRPN_DIRADR:
			SetRegAddr(GetRegAddr() + data);
			break;
		case NRPN_DIRDAT:
			SetRegData(GetRegData() + data);
			break;
		}
		//Device->SetReg(NRPNReg, data);
		break;
	case RPN_ENTRY:
		...
		{
		case RPN_PITCHBEND:
			SetBendRange(GetBendRange() + data);
			break;
		case RPN_FINETUNE:
			SetFineTune(GetFineTune() + (data * 128));
			break;
		}
		break;
	}
